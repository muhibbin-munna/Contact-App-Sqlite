...
import com.app.contactappsp.NotificationReceiver;
...
//                edit_date.setText("" +dateTemp);
            ...
            if (!user.getNotify().equals("0")) {
                ...
                if(calendar.get(Calendar.MINUTE)<10){
                    alarmTimeTv.setText("Time: " + hour + ":0" + calendar.get(Calendar.MINUTE) + " " + ap);
                }
                else {
                    alarmTimeTv.setText("Time: " + hour + ":" + calendar.get(Calendar.MINUTE) + " " + ap);
                }
            } else {
    ...
    private void setAlarm(int year, int monthAlarm, int dateAlarm, int hourAlarm, int minuteAlarm, String description) {
        ...
        String req_code = "" + contact_id + event + row_index;
    ...
    public void save(View view) {
        ...
        if (has_data.equals("null")) {
            myDatabaseHelper.insertDetailsData(String.valueOf(event), row_index, contact_id, description, remark1, remark2, notify, date, status,contact_name);
            Log.d(TAG, "onClick: add " + row_index + " " + description + " " + date + " " + status + " " + notify + " " + remark1 + " " + remark2);
            Toast.makeText(this, "added", Toast.LENGTH_SHORT).show();
        } else {
            boolean i = myDatabaseHelper.updateData(user.getId(), description, remark1, remark2, date, status,contact_name);
    ...
    public void clear(View view) {
        edit_des.setText("");
        edit_status.setText("");
        edit_remark1.setText("");
        edit_remark2.setText("");
        edit_date.setText("");
        commenceDate="";
        enableCB.setChecked(false);
        notifyCB.setSelected(false);
